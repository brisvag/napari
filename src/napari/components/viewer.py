...
from napari.components.view import View
...
Dict = dict  # rename, because Viewer
...
class Viewer(KeymapProvider, MousemapProvider, EventedModel):
    views: SelectableEventedList[View] = Field(
        default_factory=SelectableEventedList[View],
    ...
    def __getattr__(self, name):
        # TODO: can be smarter
        active = self.views.selection.active
        return getattr(name, active)
    ...
    def __init__(
        ...
        self.views.selection.events.active.connect(self._on_active_change)
        # TODO: do this smartly
        for view in self.views:
            view.layers.selection.events.active.connect(self._on_active_change)
        ...
        # TODO: this lazy import should be handled by window, not here
        from napari.window import Window
    ...
    @deprecated(
        (
            'Viewer.update_console is deprecated. Use Viewer.window.update_console instead.'
        ),
        stacklevel=2,
    )
    ...
    def json(self, **kwargs):
        """Serialize to json."""
        # Mmouse and keybindings don't belong on model
    ...
    def dict(self, **kwargs):
        ...
        # Mouse and keybindings don't belong on model
        # https://github.com/samuelcolvin/pydantic/pull/2231
        # https://github.com/samuelcolvin/pydantic/issues/660#issuecomment-642211017
        exclude = kwargs.pop('exclude', set())
        exclude = exclude.union(EXCLUDE_DICT)
    ...
    def model_dump(self, **kwargs):
        # new way of doing things for pydantic
    ...
    def _on_active_change(self, event):
        ...
        if status is not None:
            self.status, self.tooltip.text = status
        if (active := active_viewer.layers.selection.active) is not None:
            self.help = active.help
...
def current_viewer() -> Viewer | None:
    ...
    if not Viewer._instances:
        return None
    viewer = next(iter(Viewer._instances))
    if current := viewer.window.current_viewer():
        return current
    return None
    # TODO: should we return viewer instead of none?
