...
class PygfxCamera:
    def __init__(self, scene, renderer, camera_model, dims):
        self.scene = scene
        ...
        # Create 2D camera
        self._2D_camera = pygfx.OrthographicCamera()

        # Create 3D camera
        self._3D_camera = pygfx.PerspectiveCamera(fov=0)

        self._camera = self._2D_camera

        self._dims.events.ndisplay.connect(
            self._on_ndisplay_change, position='first'
        )

        # self.camera.events.center.connect(self._on_center_change)
        # self.camera.events.zoom.connect(self._on_zoom_change)
        # self.camera.events.angles.connect(self._on_angles_change)
        # self.camera.events.perspective.connect(self._on_perspective_change)
        # self.camera.events.mouse_pan.connect(self._on_mouse_toggles_change)
        # self.camera.events.mouse_zoom.connect(self._on_mouse_toggles_change)
    ...
    def _on_ndisplay_change(self):
        if self._dims.ndisplay == 3:
            pygfx.TrackballController(
                self._3D_camera, register_events=self.renderer
            )
        else:
            pygfx.PanZoomController(
                self._2D_camera, register_events=self.renderer
            )
